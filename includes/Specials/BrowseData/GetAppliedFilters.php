...
class GetAppliedFilters {
	...
	public function __invoke(): array {
		...
		foreach ( $query->appliedFilters() as $i => $af ) {
			$remainingHtml .= ( !$query->subcategory() && $i == 0 )
			...
			if ( count( $query->appliedFilters()[$i]->values ) > 1 ) {
				...
				$removeTitle = wfMessage( 'sd_browsedata_removefilter' )->text();
				$remainingHtml .= $filter_label . ' <a href="' . $remove_filter_url
					. '" title="' . $removeTitle . '"><img src="' . $sdSkinsPath . '/filter-x.png" /></a> : ';
			} else {
				$remainingHtml .= "$filter_label: ";
			}
			foreach ( $af->values as $j => $fv ) {
				if ( $j > 0 ) {
					$orText = wfMessage( 'sd_browsedata_or' )->text();
					$remainingHtml .= ' <span class="drilldown-or">' . $orText . '</span> ';
				}
				$filter_text = Utils::escapeString(
					$this->getNiceAppliedFilterValue( $af->filter->propertyType(), $fv->text )
				);
				$temp_filters_array = $query->appliedFilters();
				...
				$removeTitle = wfMessage( 'sd_browsedata_removefilter' )->text();
				$remainingHtml .= '<span class="drilldown-header-value">' . $filter_text . '</span>'
					. ' <a href="' . $remove_filter_url . '" title="' . $removeTitle
					. '"><img src="' . $sdSkinsPath . '/filter-x.png" /></a>';
			}
			...
			if ( $af->search_terms != null ) {
				foreach ( $af->search_terms as $j => $search_term ) {
					...
					$filter_text = Utils::escapeString(
						$this->getNiceAppliedFilterValue( $af->filter->propertyType(), $search_term )
					);
					$temp_filters_array = $query->appliedFilters();
					$removed_values = array_splice( $temp_filters_array[$i]->search_terms, $j, 1 );
					$remove_filter_url = $this->getUrl(
						$query->category(), $temp_filters_array, $query->subcategory()
					);
				...
				$remainingHtml .= '<span class="drilldown-header-value">'
					. $af->lower_date_string . " - " . $af->upper_date_string . '</span>';
			}
		}
